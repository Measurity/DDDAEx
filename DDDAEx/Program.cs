...
namespace DDDAEx
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            IntPtr handle = Win32Process.OpenProcess(ProcessAccess.Inject, Process.GetProcessesByName("sublime_text")[0].Id);
            IntPtr alloc = Win32Process.Alloc(handle, IntPtr.Zero, DllPath.Length, AllocationType.MemReserve | AllocationType.MemCommit, ProtectType.ReadWrite);
            Win32Process.WriteString(handle, alloc, Path.GetFullPath(DllPath));

            // Load library in target process.
            IntPtr loadLibFunc = Win32Process.GetProcAddress(Win32Process.GetModuleHandle("kernel32.dll"), "LoadLibraryA");
            ...
            var clrFuncOffset = IntPtr.Subtract(Win32Process.GetProcAddress(currentDll, "ExecuteCLR"), currentDll.ToInt32());
